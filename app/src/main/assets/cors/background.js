...
browser.webRequest.onBeforeSendHeaders.addListener(
    function (request) {
        ...
        for (let header of request.requestHeaders)
            bgReqHeader[request.requestId][header.name.toLowerCase()] = header.value;
...
browser.webRequest.onHeadersReceived.addListener(
    function (response) {
        ...
        const replaceHeaders = {
            ...
            'access-control-allow-methods': requestHeaders['access-control-request-method'] || false,
            'access-control-allow-headers': requestHeaders['access-control-request-headers'] || false,
        ...
        for (let header of response.responseHeaders) {
            ...
            if (lowerName in replaceHeaders) {
                if (replaceHeaders[lowerName])
                    responseHeaders.push({name: header.name, value: replaceHeaders[lowerName]});
        ...
        for (let lowerName in replaceHeaders) {
            if (replaceHeaders[lowerName])
                responseHeaders.push({name: lowerName, value: replaceHeaders[lowerName]});
        }
